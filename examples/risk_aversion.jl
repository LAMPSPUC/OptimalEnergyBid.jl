...
for t in 1:(prb.numbers.duration), i in 1:(prb.numbers.units)
    prb.random.prices_real_time[t][i] = [1.5 - t * 0.1, 2.0 + t * 10]
    prb.data.prices_real_time_curve[t][i] = [1.5 - t * 0.1, 2.0 + t * 10]
end
...
OptimalEnergyBid.set_parameter!(prb, OptimalEnergyBid.Parameter.Lambda, 0.5)
